...
def _httpx_verify() -> bool | str:
    """Return the configured CA bundle or keep httpx verification enabled."""

    return settings.ECHOKEY_CA_CERT or True
...
class EchoKeyClient:
    ...
    def poll(
        ...
        start = time.time()
        with httpx.Client(timeout=10.0, verify=_httpx_verify()) as client:
